package com.vulcanth.reports.bungee.comandos.cmds;
...
public class ReportarCommand extends Commands {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length == 1) {
            ...
            Bungee.getInstance().getReportDao().createReport(player.getName(), target.getName(), target.getServer().getInfo());
            ProxyServer.getInstance().getPlayers().stream().filter(op -> op.hasPermission("vulcanth.punir")).forEach(op -> {
                ...
                teleport.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/btp " + target.getName()));
                ...
                op.sendMessage(TextComponent.fromLegacyText("§c§lREPORT NOVO ENVIADO" +
                        ...
                        "\n§cServidor: §f" + Bungee.getInstance().getProxy().getPlayer(target.getName()).getServer().getInfo().getName()));
                op.sendMessage(teleport);
                op.sendMessage(TextComponent.fromLegacyText(" "));
            });

            player.sendMessage(TextComponent.fromLegacyText(" "));
            player.sendMessage(TextComponent.fromLegacyText("§a* Você reportou o jogador " + Role.getPrefixed(target.getName()) +"§a. Um membro de nossa equipe §afoi notificado e o " +
                    "comportamento deste jogador §aserá analisado em breve.\n\n §a* O uso abusivo deste comando poderá §aresultar em punição."));
            player.sendMessage(TextComponent.fromLegacyText(" "));
            cooldown.add(player);
            ProxyServer.getInstance().getScheduler().schedule(Bungee.getInstance(), () -> cooldown.remove(player), 2, TimeUnit.SECONDS);
        }
    }
    ...
    private static boolean impossibleToBan(String nickName) {
        return Stream.of("kNyel", "oPewdBR", "_KyuraKing_").anyMatch(s -> s.equalsIgnoreCase(nickName));
    }
