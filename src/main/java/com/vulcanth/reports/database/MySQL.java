package com.vulcanth.reports.database;
...
public class MySQL extends Database{
        ...
        if (!isConnected()) {
            try {
                ...
                connection = DriverManager.getConnection("jdbc:mysql://" + host + ":" + port + "/" + database
                        ...
                        username, password);
                if (bol) {
                    Main.getInstance().getLogger().info("§aConectado ao MySQL!");
                    return;
                }

                Main.getInstance().getLogger().info("§6Reconectado ao MySQL!");
    ...
    @Override
    public PreparedStatement execute(String sql, Object... vars) {
        executor.execute(() -> {
            update(sql, vars);
        });
        return null;
    ...
    public static boolean checarExistePlayer(String nome) {
        try {
            PreparedStatement ps = Database.getInstance().execute("SELECT * FROM VulcanthReports WHERE id = ?");
